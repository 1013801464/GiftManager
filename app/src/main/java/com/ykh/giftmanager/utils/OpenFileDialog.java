...
public class OpenFileDialog {
    ...
    public static Dialog createDialog(Context context, View title, CallbackBundle callback, String suffix, Map<String, Integer> images) {
...
//                list.add(map);
                ...
                map.put("name", "..");
                map.put("path", "返回上一级");
                map.put("last_level_path", new File(path).getParent());
            ...
            for (File file : files) {
                if (file.isDirectory() && file.listFiles() != null) {
                    ...
                    map.put("last_level_path", file.getPath());
                    int childLength = file.listFiles().length;
                    map.put("path", childLength + " 个项目");
                    if (childLength > 0)
                        map.put("img", getImageId(sFolder));
                    else
                        map.put("img", getImageId("empty_folder"));
                    lfolders.add(map);
                } else if (file.isFile()) {
                    // 添加文件
                    ...
                    if (suffix == null || suffix.length() == 0 || (sf.length() > 0 && suffix.indexOf("." + sf + ";") >= 0)) {
                        Map<String, Object> map = new HashMap<String, Object>();
                        map.put("name", file.getName());
                        map.put("path", FileSizeUtil.FormatFileSize(file.length()));
                        map.put("last_level_path", file.getPath());
        ...
        public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
            // 条目选择
            String pt = (String) list.get(position).get("last_level_path");
...
//                }
                ...
                if (fl.isFile()) {
                    // 如果是文件
                    if (isDebugging) Log.e(TAG, "onItemClick: " + dlg);
                    if (dlg != null) {
                        dlg.dismiss();
                    }
